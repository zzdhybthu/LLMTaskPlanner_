...
# nohup python -u src/evaluate.py --config-name=config_sllm planner.model_name=$model_name planner.use_feedback=True planner.cot=True task_category=$task_category start_from=4 num_tasks=1 2>&1 | grep -v "HTTP Request: POST" >> $log_file &
nohup python -u src/evaluate.py --config-name=config_sllm planner.model_name=$model_name planner.use_feedback=True planner.cot=True task_category=$task_category 2>&1 | grep -v "HTTP Request: POST" >> $log_file &
pid=$!
