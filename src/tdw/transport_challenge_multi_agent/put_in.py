...
from src.tdw.transport_challenge_multi_agent.reach_for_transport_challenge import ReachForTransportChallenge
from src.tdw.transport_challenge_multi_agent.challenge_state import ChallengeState
from src.tdw.transport_challenge_multi_agent.replicant_target_position import ReplicantTargetPosition, POSITIONS
from src.tdw.transport_challenge_multi_agent.reset_arms import ResetArms
from src.tdw.transport_challenge_multi_agent.multi_action import MultiAction
...
class PutIn(MultiAction):
    ...
    def __init__(self, dynamic: ReplicantDynamic, state: ChallengeState):
        ...
        log.info(dynamic.held_objects)
        log.info(state.container_ids)
        log.info(state.target_object_ids)
        ...
        for arm in [Arm.left, Arm.right]:
            if arm in dynamic.held_objects and dynamic.held_objects[arm] in state.target_object_ids:
                ...
                break
        log.info(self._container_arm, self._object_arm)
    ...
    def get_ongoing_commands(self, resp: List[bytes], static: ReplicantStatic, dynamic: ReplicantDynamic) -> List[dict]:
        ...
        if self._sub_action.status == ActionStatus.ongoing:
            ...
            if self._put_in_state == _PutInState.moving_object_arm_elbow_away:
                log.info("moving container in front of replicant")
            ...
            # Move the object over the container.
                ...
                log.info("moving object over container")
