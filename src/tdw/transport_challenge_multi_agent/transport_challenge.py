...
from src.tdw.transport_challenge_multi_agent.challenge_state import ChallengeState
from src.tdw.transport_challenge_multi_agent.replicant_transport_challenge import ReplicantTransportChallenge
from src.tdw.transport_challenge_multi_agent.paths import CONTAINERS_PATH, TARGET_OBJECTS_PATH, TARGET_OBJECT_MATERIALS_PATH
from src.tdw.transport_challenge_multi_agent.globals import Globals
from src.tdw.transport_challenge_multi_agent.asset_cached_controller import AssetCachedController
...
class TransportChallenge(AssetCachedController):
    ...
    def __init__(self, port: int = 1071, check_version: bool = True, launch_build: bool = True, screen_width: int = 256,
                 screen_height: int = 256, image_frequency: ImageFrequency = ImageFrequency.once, png: bool = True, asset_cache_dir = "cache/transport_challenge_asset_bundles",
        ...
        try:
            ...
            if p.stem != "Co-LLM-Agents":
                log.info("Warning! You might be using code copied from the Co-LLM-Agents repo. Your code might be out of date.\n")
        except OSError:
            pass
        if TransportChallenge.TDW_VERSION != __version__:
            log.info(f"Warning! Your local install of TDW is version {__version__} but the Multi-Agent Transport Challenge requires version {TransportChallenge.TDW_VERSION}\n")
    ...
    def _start_trial_new(self, replicants: Union[int, List[Union[int, np.ndarray, Dict[str, float]]]] = 2, task_type = 'food', random_seed: int = None) -> None:
        ...
        log.info(f"{load_count_and_position_path} saved")
