...
def generate_imgs(save_dir, data_prefix, scene, layout, task):
    ...
    _start = time.time()
    log.info("Start ---------------------------------", time.strftime("%H:%M:%S", time.localtime(_start)))
    ...
    log.info("Build Scene ---------------------------------", time.time() - _start)
    ...
    log.info("Setting -------------------------------", time.time() - _start)
    ...
    log.info(time.time() - _start)

    log.info("Move ----------------------------------", time.time() - _start)
    random_sample(c, obj_ids, save_dir)
    c.communicate({"$type": "terminate"})


def random_sample(c: TransportChallenge, obj_ids, save_dir):
            ...
            while (step_count <= 500) and (try_count <= 1000):
                ...
                while replicant.action.status in [ActionStatus.ongoing]:
                    ...
                    if not np.all(pre_pos == cur_pos):
                        ...
                        pre_pos = cur_pos
                c.communicate([])
                log.info(replicant.action.status, step_count, try_count)
